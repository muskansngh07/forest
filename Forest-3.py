...
class Tree(Entity):
    def __init__(self, params, species_id=1, **kwargs):
        ...
        # ── FIX A: manual cylinder mesh (built-in not available) ──────
        # Build a simple tapered cylinder by hand using triangles.
        sv, st, sn = [], [], []
        for i in range(STEM_STACKS):
            y1 = (i / STEM_STACKS) * H
            y2 = ((i + 1) / STEM_STACKS) * H
            r1 = params.stem_radius_at_height(y1, H)
            r2 = params.stem_radius_at_height(y2, H)
            base_idx = len(sv)
            for j in range(STEM_SLICES + 1):
                angle = 2 * math.pi * j / STEM_SLICES
                ca, sa = math.cos(angle), math.sin(angle)
                sv.append((r1 * ca, y1, r1 * sa))
                sv.append((r2 * ca, y2, r2 * sa))
                sn.append((ca, 0, sa))
                sn.append((ca, 0, sa))
            for j in range(STEM_SLICES):
                idx = base_idx + j * 2
                st.append((idx, idx + 1, idx + 2))
                st.append((idx + 1, idx + 3, idx + 2))
        if sv:
            trunk_e       = Entity(parent=self, color=bark)
            trunk_e.model = Mesh(vertices=sv, triangles=st, normals=sn)

        # ── FIX B + C: billboard cross, scaled down, transparent ───────
        # render_queue must be an integer in this Ursina version (not string)
        # 1 = renders after opaque objects so transparency blends correctly
        for rot in (0, 90):
            Entity(
                parent       = self,
                model        = 'quad',
                ...
                rotation_y   = rot,
                double_sided = True,
                render_queue = 1,             # FIX B: integer, not string
            )
